...
export const useAdvancedChatFilter = (
  ...
  isSenderTalent: boolean = false, // ✅ NEW: The hook now knows the sender's role
) => {
  ...
  // ✅ UPDATED: This function now generates different messages based on the sender's role.
  const generateBlockReason = (patterns: string[]): string => {
    const isPhone = patterns.includes("split_phone") || patterns.includes("number_sequence");
  ...
  // ✅ UPDATED: This function also generates different messages now.
